...
MainWindow::MainWindow(QWidget *parent)
    ...
    , m_currentMoleIndex(VALUE_255)
    , m_previousMoleIndex(VALUE_255)
...
void MainWindow::initUI()
{
   ...
   // m_hitSound->setSource(QUrl::fromLocalFile("hit.wav")); // 可以换成资源 :/hit.wav
    ...
    layout->setSpacing(VALUE_4);
    layout->setContentsMargins(VALUE_10,VALUE_10,VALUE_10,VALUE_10);
    // 正确设置网格拉伸：3列3行 填满屏幕
    for(int i =INDEX_0; i < INDEX_3; i++){
        layout->setColumnStretch(i,VALUE_1);
    }

    for(int i =INDEX_0; i < INDEX_3; i++){
        layout->setRowStretch(i+VALUE_1,VALUE_1);
    }
    ...
    for(int i=INDEX_0; i<INDEX_9; i++){
        ...
        btn->setContentsMargins(0,0,0,0); // 消除按钮内边距
        btn->setStyleSheet(R"(
            QPushButton {
                border: none;
                border-radius: 8px;
                background-image: url(:/mole_hole_1.png);
        ...
        layout->addWidget(btn, i/VALUE_3+VALUE_1, i%VALUE_3);
    }
    ...
    layout->addWidget(m_restartBtn, VALUE_4,VALUE_0,VALUE_1,VALUE_4);
}
...
void MainWindow::startGame()
{
    ...
    m_scoreLabel->setText(QString("分数: %1").arg(VALUE_0));
    m_timeLabel->setText(QString("时间: %1").arg(VALUE_30));
...
{
    // 先隐藏上一个地鼠
    if(m_currentMoleIndex != VALUE_255){
    ...
    m_moleBtns[m_currentMoleIndex]->setStyleSheet(R"(
        QPushButton {
            border: none;
            border-radius: 8px;
            background-image: url(:/mole_1.png);
            background-repeat: no-repeat;
            background-position: center;
...
void MainWindow::whackMole()
{
    ...
    mole->setStyleSheet(R"(
        QPushButton {
            border: none;
            border-radius: 8px;
            background-image: url(:/mole_hit_1.png);
            background-repeat: no-repeat;
            background-position: center;
